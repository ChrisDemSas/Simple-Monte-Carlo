...
class MonteCarlo:
    ...
    def visualize_sample(self, lower_limit: int, upper_limit: int):
        ...
        sns.lineplot(samples, dataset)
        print(f'Mean {np.mean(dataset)} and Variance {np.var(dataset)}')
    
    def visualize_parameter(self, lower_limit: int, upper_limit: int):
        """Visualize the results on the variation of parameters."""
        
        parameter = [n for n in range(lower_limit, upper_limit + 1)]
        data = []
        ...
        for d in data:
            sns.lineplot(parameter, d[1]).set(title = d[0])
...
if __name__ == '__main__':
    ...
    function = lambda x, lamda: 1/3 * np.exp(lamda) * x**4 * np.cos(x**2)
    upper = np.inf
    lower = 0
    
    mc = MonteCarlo(function, upper, lower, 10000)
    mc.set_distribution('Exponential', param)
    answer = mc.evaluate()
    print(answer)
    # mc.visualize_sample(1, 1000)
    # mc.visualize_parameter(1, 10)
